...
def _selftest() -> int:
    """Print camera/backend diagnostics and exit (ISCOUT_SELFTEST=1)."""
    from src import camera_backend
    print("platform macos:", camera_backend.is_macos())
    devs = camera_backend.list_avf_devices()
    print(f"AVFoundation video devices ({len(devs)}):")
    for d in devs:
        print(f"  [{d['index']}] {d['name']!r} thermal={d['is_thermal']} "
              f"id={d['unique_id']}")
    return 0
...
def main() -> int:
    ...
    if os.environ.get("ISCOUT_SELFTEST") == "1":
        return _selftest()
